...
Release CI fetches KrkrPatch revision `587261001bf95feab4f0f1cbcbd22cfdadb97e31`, applies `patches/KrkrPatch-DetourRestoreAfterWith.patch`, and builds the x86 loader and DLL. CI also builds the checked-in x86 `steam_api.dll` proxy and packages both startup methods in `LocalizationSetup.exe`. The installer always shows the target path and lets the user choose between direct Steam launch and the separate KrkrPatch loader.

Every push to `main` uploads `LocalizationSetup.exe` as a workflow artifact for testing. Publishing a GitHub release runs the same build, attaches the installer, matching source, and licence archive, then writes their SHA-256 hashes directly into the release description.

The Steam startup method preserves the verified original as `steam_api_original.dll` and forwards all 1,069 exports to it. Localization starts separately after Steam initialization reaches the proxy. Before initializing KrkrPatch, the proxy verifies the exact supported game EXE, `patch.xp3`, and original Steam DLL SHA-256 hashes. Missing files, hash mismatches, DLL load failures, invalid KrkrPatch configuration, and hook-installation failures are logged and fall back to the unlocalized game. `patchNoProtocol` remains enabled because this game can request scenario files by bare storage name. Release configuration uses `logLevel` `1`.
...
Official releases need a publicly trusted Authenticode identity. After selecting a signing provider, configure signing only in a protected GitHub release environment: sign and RFC 3161 timestamp `KrkrPatchLoader.exe`, `KrkrPatch.dll`, and `steam_api.dll`, verify them, compile the installer, then sign and verify `LocalizationSetup.exe`. Generate checksums only after signing. Inno Setup's generated uninstaller requires `SignedUninstaller=yes` and signing access during compilation if it must also carry the publisher signature.
